      ...
      <div className="flex items-center justify-between py-3 border-b border-border last:border-0">
        <div className="flex items-center gap-3 min-w-0">
          <Link
            ...
            className="font-medium text-ghost hover:text-emerald transition-colors truncate"
          >
            {contact.first_name} {contact.last_name}
          </Link>
          {contact.source && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-surface text-ghost-muted border border-border shrink-0">
              {contact.source}
            </span>
          )}
          {createdAgo && (
            <span className="text-xs text-ghost-muted hidden sm:inline shrink-0">
              {createdAgo}
            </span>
          )}
        </div>
